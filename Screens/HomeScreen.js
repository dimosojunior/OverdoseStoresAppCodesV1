...
import PeopleCategoriesComponent from '../RenderedComponents/PeopleCategoriesComponent';
...
const getItems = () => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetPeopleCategories/?page=${current_page}&page_size=2`
    // console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
  ...
  <>
      ...
      <>
        <FlatList
          ...
          renderItem={({index, item}) => <PeopleCategoriesComponent item={item} />}
          numColumns={2}
