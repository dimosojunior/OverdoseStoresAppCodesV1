...
const ReportsForSpecificCategory = ({navigation, route}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/GetAllProductsOrdersByProductsCategories/?id=${id}&page=${current_page}&page_size=2`;
...
useEffect(() => {
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/FilterProductsOrdersByProductsCategories/?id=${id}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
export default ReportsForSpecificCategory;
