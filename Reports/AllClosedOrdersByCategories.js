...
const AllClosedOrdersByCategories = ({navigation, route}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/ClosedProductsOrdersByProductCategories/?id=${id}&page=${current_page}&page_size=2`;
...
useEffect(() => {
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/FilterClosedOrdersByProductsCategoriesView/?id=${id}&startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
  ...
  <MinorHeader title="Reports"/>
      ...
      <View style={{marginVertical: 12}}>
        ...
        style={globalStyles.OverdoseOurProductsHeaderText}
         >
          All Approved Orders - ({CategoryName})
        </Text>
...
export default AllClosedOrdersByCategories;
