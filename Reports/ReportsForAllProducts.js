...
const ReportsForAllProducts = ({navigation}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/GetAllProductsOrdersWithoutProductsCategories/?page=${current_page}&page_size=2`;
...
useEffect(() => {
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/FilterProductsOrdersWithoutProductsCategories/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
export default ReportsForAllProducts;
